    ...
    module Acts #:nodoc:
        module Track #:nodoc:
            ...
            module InstanceMethods
                ...
                def track_create
                    Tracker.current = User.find(created_by) if defined? created_by and not Tracker.current
                    write_audit_for_create(:action => 'create', :changes => audited_attributes)
                end
                def track_update
                    unless (changes = audited_changes).empty?
                        Tracker.current = User.find(updated_by) unless defined? updated_by and not Tracker.current
...
#module to e included in the application controller
module Akamai
  module Track
    module Setup 
